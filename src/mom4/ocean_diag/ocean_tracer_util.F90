...
subroutine tracer_min_max(Time, Tracer)
  ...
  wrk1(isd:ied,jsd:jed,:) = Tracer%field(isd:ied,jsd:jed,:,tau)
  
  iminarr=minloc(wrk1,Grd%tmask > 0.)
  imaxarr=maxloc(wrk1,Grd%tmask > 0.)
  itmin=iminarr(1)+isd-1
  jtmin=iminarr(2)+jsd-1
  ktmin=iminarr(3)
  itmax=imaxarr(1)+isd-1
  jtmax=imaxarr(2)+jsd-1
  ktmax=imaxarr(3)
